###CODE FOR BRUEELIA PARAFIT ANALYSIS
...
#tree <- read.tree('Brueelia_bGMYC_random100.trees')
...
h.pats <- as.matrix(cophenetic(h.tree))
...
#CONVERT TO A MATRIX
#brueelia_mx <- as.matrix(brueelia_mx)

#Run ParaFit
#brpf <- parafitparafit(h.pat, p.pat, brueelia_mx, nperm=1, test.links=TRUE, correction='lingoes')
...
par(mar=c(1,1,1,1))
cophyloplot(h.tree, p.tree, assoc=links, 
            show.tip.label=F, # too long taxa names...
            space=400, use.edge.length=FALSE, gap=0.5, rotate=T, col='red')